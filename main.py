    ...
    MDBoxLayout:
        ...
        ScrollView:
            MDBoxLayout:
                ...
                MDCard:
                    size_hint_y: None
                    height: "250dp" if Window.width >= dp(500) else "310dp"
                    radius: [30]
                    elevation: 8
                    md_bg_color: "#5B2EFF"
                    padding: "25dp"
                    ...
                    MDBoxLayout:
                        ...
                        MDIcon:
                            icon: "school"
                            font_size: "120sp" if Window.width >= dp(500) else "70sp"
                            halign: "center"
                            theme_text_color: "Custom"
                            text_color: 1,1,1,1
                        ...
                        MDBoxLayout:
                            ...
                            MDLabel:
                                text: "Go To Space Tomorrow With Tuskanic."
                                bold: True
                                font_style: "H4"
                                theme_text_color: "Custom"
                                text_color: 1,1,1,1
                            ...
                            MDLabel:
                                text: "Best online and offline courses for your future."
                                theme_text_color: "Custom"
...
# Screens other than "main" are NOT declared in the KV MDScreenManager
# block anymore. Building all 14 KivyMD screens (cards, lists, labels...)
# up front made startup noticeably slow/laggy, especially on debug builds.
# Instead each screen class is created lazily, on the first time the user
# actually navigates to it, via this name -> class lookup table.
SCREEN_CLASSES = {
    ...
    "chat_notifications": ChatNotificationsScreen,
...
class TuskanicApp(MDApp):
    ...
    def _sync_dark_mode_switch(self, dt):
        try:
            self.root.get_screen("main").ids.dark_mode_switch.active = (
    ...
    def go_to(self, screen):
        # build the screen the first time it's actually visited, instead
        # of every screen being built eagerly at app startup
        if screen not in self.root.screen_names and screen in SCREEN_CLASSES:
            self.root.add_widget(SCREEN_CLASSES[screen]())
    ...
    def go_back(self):
        if len(self.history) > 1:
            self.history.pop()
            self.root.current = self.history[-1]
...
TuskanicApp().run() kivy.lang import Builder
from kivy.core.window import Window
from kivy.metrics import dp
from kivy.clock import Clock
from kivy.animation import Animation
